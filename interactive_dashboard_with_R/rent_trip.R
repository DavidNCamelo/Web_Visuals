...
trips <- read.csv("./data/trip_data.csv")
stations <- read.csv("./data/station_data.csv")
...
trips <- trips %>%
  mutate(
    across(
      ...
      ~ ifelse(
        .x %in% as.integer(names(new_station_codes)),
...
trips <- trips %>%
  left_join(
    stations %>% select(Id, Name, City),
    by = c("Start.Station" = "Id")
  ) %>%
...
trips <- trips %>%
  ...
  mutate(
    # Separate date columns in two components
